...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      await storageService.init(); // Wait for IndexedDB
  ...
  useEffect(() => {
    if (isSettingsOpen) {
      ...
      const limitKB = 1048576; // 1GB Virtual Limit for UI (IndexedDB is actually much larger)
